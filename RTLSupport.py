...
class RTLSupport(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.title = "RTL Support"
        self.parent.categories = ["Utilities"]
        self.parent.dependencies = []
        self.parent.contributors = ["Mohamed Alalli Bilal"]
        self.parent.helpText = """
        This module adds RTL (Right-to-Left) support to 3D Slicer.
        """
        self.parent.acknowledgementText = """
        This module was developed by Mohamed Alalli Bilal.
        """

class RTLSupportWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        # Create button using standard QPushButton with RTL support
        self.enableRTLButton = qt.QPushButton("Enable RTL Support")
        self.enableRTLButton.toolTip = "Enable Right-to-Left (RTL) language support"
        self.enableRTLButton.setMinimumSize(200, 50)
        
        # Apply RTL styles
        self.enableRTLButton.setLayoutDirection(qt.Qt.RightToLeft)
        self.enableRTLButton.setStyleSheet("""
            QPushButton {
                min-width: 200px;
                min-height: 50px;
                font-size: 14px;
        ...
        container.layout().addWidget(self.enableRTLButton)
        ...
        self.layout.addWidget(self.enableRTLButton)
        
        # Connect button to function
        self.enableRTLButton.connect('clicked(bool)', self.onEnableRTL)
    ...
    def onEnableRTL(self):
        ...
        try:
            ...
            # Show confirmation message in RTL
            msgBox = qt.QMessageBox()
            msgBox.setWindowTitle("تم التفعيل")
            msgBox.setText("تم تفعيل دعم اللغة العربية (من اليمين إلى اليسار)")
            msgBox.setLayoutDirection(qt.Qt.RightToLeft)
            ...
            import traceback
            error_msg = f"حدث خطأ أثناء تفعيل وضع RTL:\n{str(e)}\n\n{traceback.format_exc()}"
            qt.QMessageBox.critical(None, "خطأ", error_msg)
...
class RTLSupportTest(ScriptedLoadableModuleTest):
    """
    ...
    def runTest(self):
        ...
        self.setUp()
        self.test_RTLSupport1()

    def test_RTLSupport1(self):
        """Basic test that verifies the module can be loaded."""
        self.delayDisplay("Testing RTL Support")

        # Test the module by creating a widget and clicking the button
        widget = RTLSupportWidget(None)
        widget.setup()
        
        # Store current layout direction to restore later
        app = qt.QApplication.instance()
        originalLayoutDirection = app.layoutDirection()
        
        try:
            # Test RTL enable
            widget.onEnableRTL()
            self.assertEqual(app.layoutDirection(), qt.Qt.RightToLeft)
            self.delayDisplay('Test passed: RTL enabled successfully')
            
            # Test RTL application to widgets
            testWidget = qt.QWidget()
            testWidget.show()
            widget.applyRTLToWidget(testWidget)
            self.assertEqual(testWidget.layoutDirection(), qt.Qt.RightToLeft)
            self.delayDisplay('Test passed: RTL applied to widgets')
            
        finally:
            # Restore original layout direction
            app.setLayoutDirection(originalLayoutDirection)
